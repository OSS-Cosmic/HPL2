...
namespace hpl {
    ...
    void UpdateEventLoop::Subscribe(BroadcastEvent event, UpdateEventLoop::UpdateEvent::Handler& handler) {
        handler.Connect(m_events[static_cast<size_t>(event)]);
    }

    void UpdateEventLoop::Subscribe(
        const std::string_view groupName, BroadcastEvent event, UpdateEventLoop::UpdateEvent::Handler& handler) {
        ASSERT(static_cast<size_t>(event) < m_events.size() && "Event out of range");
